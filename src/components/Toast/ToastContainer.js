...
class ToastContainer extends Component {
  ...
  canBeRendered(content) {
    return (
      isValidElement(content) || typeof content === 'string' || typeof content === 'number' || typeof content === 'function'
    )
  }
  ...
  show(content, options) {
    ...
    this.setState(
      {
        toast: (toastOptions.updateId ? [...this.state.toast] : [...this.state.toast, toastId]).filter(
          id => id !== options.staleToastId
        )
      },
      this.dispatchChange
    )
  ...
  makeToast(content, options) {
    return (
      <Toast {...options} isDocumentHidden={this.state.isDocumentHidden} key={`toast-${options.key}`}>
        {content}
      </Toast>
    )
  }
  ...
  renderToast() {
    ...
    return Object.keys(toastToRender).map(position => {
      const disablePointer = toastToRender[position].length === 1 && toastToRender[position][0] === null
      const props = {
        className: cx(
